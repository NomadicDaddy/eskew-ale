...
select top (@topcount)
	[creation_time],
	[last_execution_time],
	[row] = Rank() over (order by ([total_worker_time] + 0.0) / [execution_count] desc, [sql_handle], [statement_start_offset]),
	[l1] = (Rank() over (order by ([total_worker_time] + 0.0) / [execution_count] desc, [sql_handle], [statement_start_offset])) % 2,
	[total_worker_time] = ([total_worker_time] + 0.0) / 1000,
	[AvgCPUTime] = ([total_worker_time] + 0.0) / ([execution_count] * 1000),
	[LogicalReads] = [total_logical_reads],
	[LogicalWrites] = [total_logical_writes],
	[execution_count],
	[AggIO] = [total_logical_reads] + [total_logical_writes],
	[AvgIO] = ([total_logical_reads] + [total_logical_writes]) / ([execution_count] + 0.0),
	[query_text] = case
	...
	st.[objectid],
	[objName] = object_name(st.[objectid])
from
...
order by
	case
		when @type = 'time' then [total_worker_time]
		when @type = 'cpu' then ([total_worker_time] + 0.0) / ([execution_count] * 1000)
		when @type = 'count' then ([execution_count])
		when @type = 'avgio' then ([total_logical_reads] + [total_logical_writes]) / ([execution_count] + 0.0)
		else [total_logical_reads] + [total_logical_writes]
	end desc
end try
