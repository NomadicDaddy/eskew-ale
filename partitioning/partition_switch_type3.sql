...
alter table dbo.[OrderTracking_parByDate_staging] with check add constraint [matchRange] check ([EventDateTime] is not null and [EventDateTime] >= '2013-01-01 00:00:00.000' and [EventDateTime] < '2014-01-01 00:00:00.000') ;
go
...
select [source] = Count(*) from dbo.[OrderTracking_parByDate] where [EventDateTime] >= '2013-01-01 00:00:00.000' and [EventDateTime] < '2014-01-01 00:00:00.000' ;
select [target] = Count(*) from dbo.[OrderTracking_parByDate_staging] ;

-- switch to target partition
alter table dbo.[OrderTracking_parByDate] switch partition 4 to [OrderTracking_parByDate_staging] ;
go
...
alter partition function [pf_OrderTracking_parByDate] () merge range ('20130101') ;
go
